...
def judging_status_from_resp(resp):
    filtered_labels = ["Car", "Vehicle", "Transportation", "Toy", "Switch", "Electrical Device", "Tarmac", "Skateboard", "Hand", "Arm", "Human", "Person"]
    breaker=False
    for label_dict in resp["Labels"]:
        # print(label_dict["Name"])
        for filtered_label in filtered_labels:
            if label_dict["Name"] == filtered_label:
                print(filtered_label, "detected!")
                return 1
                breaker=True
                print(breaker)
                break
...
# 무한루프
while cap.isOpened():
    cap.set(cv2.CAP_PROP_POS_FRAMES, count*500)
    ret, img = cap.read()     # 카메라로부터 현재 영상을 받아 img에 저장, 잘 받았다면 ret가 참
